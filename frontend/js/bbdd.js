...
let bbddSortDir     = 'asc';
let isAdmin         = false;  // se rellena al iniciar la pestaña
...
async function initBBDDTab() {
  // Comprobar si el usuario es admin (rol viene del servidor Node via checkAuth)
  isAdmin = window.currentUser?.role === 'admin';

  // Mostrar/ocultar sección de subida según rol
  ...
  if (roleBadge) {
    roleBadge.innerHTML = isAdmin
      ? `<span class="badge badge-lime"> Administrador</span>`
...
//  KPIs DE LA BBDD
// Los KPIs se obtienen del servidor (stats de toda la tabla),
// independientemente de cuántos registros haya cargado en la vista local.
async function mostrarBBDDStats() {
  ...
  grid.innerHTML =
    statCard('lime',   '…', 'Licitaciones en BBDD', 'registros totales') +
    statCard('cyan',   '…', 'Importe total',        'con IVA') +
    statCard('green',  '…', 'Con adjudicatario',    'de …') +
    statCard('orange', '…', 'Órganos contratantes', 'distintos') +
    statCard('amber',  '…', 'Adjudicatarios',       'distintos');

  try {
    const resp = await fetch('/api/sync/stats', { credentials: 'include' });
    if (!resp.ok) throw new Error(`Error del servidor (${resp.status})`);
    const json = await resp.json();
    if (!json.success) throw new Error(json.error || 'Error al cargar estadísticas');

    const { total, importeTotal, adjudicados, organos, adjudicatarios } = json.stats;

    grid.innerHTML =
      statCard('lime',   total,                 'Licitaciones en BBDD', 'registros totales') +
      statCard('cyan',   formatEUR(importeTotal),'Importe total',        'con IVA') +
      statCard('green',  adjudicados,            'Con adjudicatario',    `de ${total}`) +
      statCard('orange', organos,                'Órganos contratantes', 'distintos') +
      statCard('amber',  adjudicatarios,         'Adjudicatarios',       'distintos');

  } catch (err) {
    console.warn('[bbdd] No se pudieron cargar stats globales, usando datos locales:', err);
    // Fallback: calcular desde los datos locales cargados
    const total          = bbddData.length;
    const importe        = bbddData.reduce((s, r) => s + (r._importeConIVA || 0), 0);
    const adjudicados    = bbddData.filter(r => r.adjudicatario).length;
    const organos        = new Set(bbddData.map(r => r.organo).filter(Boolean)).size;
    const adjudicatarios = new Set(bbddData.map(r => r.adjudicatario).filter(Boolean)).size;

    grid.innerHTML =
      statCard('lime',   total,              'Licitaciones en BBDD', 'registros locales') +
      statCard('cyan',   formatEUR(importe), 'Importe total',        'con IVA') +
      statCard('green',  adjudicados,        'Con adjudicatario',    `de ${total}`) +
      statCard('orange', organos,            'Órganos contratantes', 'distintos') +
      statCard('amber',  adjudicatarios,     'Adjudicatarios',       'distintos');
  }
...
async function exportarCopiaSeguridad() {
  if (!isAdmin) {
    setBBDDStatus('error', 'Solo administradores pueden crear copias de seguridad.');
    return;
  }
  ...
  try {
    ...
    a.href     = url;
    a.download = `licitlab-backup-${isoStamp}.json`;
...
function restaurarDesdeCopia() {
  if (!isAdmin) {
    setBBDDStatus('error', 'Solo administradores pueden restaurar copias de seguridad.');
    return;
  }
  ...
  input.addEventListener('change', async () => {
    ...
    reader.onload = async (e) => {
      ...
      if (!payload._meta || payload._meta.source !== 'licit-lab-backup' || !Array.isArray(payload.rows)) {
        setBBDDStatus('error', 'El archivo no parece una copia de seguridad de LICIT·LAB (formato incorrecto).');
        return;
      }
        ...
        ${data.map(row => {
          ...
          <tr data-batch-id="${row.id}" style="border-bottom:1px solid var(--border)">
            ...
            <td style="padding:7px 8px;text-align:center">
              <button title="Eliminar este registro del historial"
