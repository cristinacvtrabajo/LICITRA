...
function populateFilters() {
  ...
  populateSelect('filterVigencia',      uniqueVals('vigencia'));
  ...
  if (!window._filtersBound) {
    ...
    ['filterEstado','filterVigencia','filterRangoFechas']
...
function applyFilters() {
  ...
  filteredData = allData.filter(r => {
    ...
    if (vigencia && r.vigencia      !== vigencia) return false;
    ...
    if (fechaDesde || fechaHasta) {
      const fechaStr = r.primeraPublicacion || r.fechaActualizacion || r.fechaAcuerdo || '';
    ...
    const importe = r._importeConIVA || 0;
    if (importe < minVal || importe > maxVal) return false;
...
function _updateActiveFilterBadge() {
  const allIds = ['filterEstado','filterVigencia','filterRangoFechas',
...
function resetFilters() {
  ...
  ['filterEstado','filterVigencia','filterRangoFechas'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  // Limpiar comboboxes
...
function renderBody(cols) {
  ...
  document.getElementById('tableBody').innerHTML = page.map((row, ri) => {
    const cells = cols.map(c => {
      ...
      } else if (c.key === 'vigencia') {
        display = badgeVigencia(val);
...
function openModal(idx) {
  ...
  const fields = [
    ...
    { key: 'primeraPublicacion',label: 'Primera publicación' },
