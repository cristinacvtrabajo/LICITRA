...
/** Limpia un texto para que sea seguro meterlo en el prompt */
function groqSanear(val) {
...
function obtenerMejorUrl(licit) {
  ...
  // Prioridad 3: Construir URL a partir del expediente
  if (licit.expediente && licit.expediente.trim()) {
    const expedienteLimpio = licit.expediente.replace(/\s/g, '').replace(/[^\w\d-]/g, '');
    if (expedienteLimpio.length > 3) {
      return { 
        url: `https://contrataciondelestado.es/licitacion?id=${encodeURIComponent(expedienteLimpio)}`,
  ...
  // Prioridad 4: Búsqueda por órgano
  if (licit.organo && licit.organo.trim()) {
    return {
      url: `https://contrataciondelestado.es/buscar?q=${encodeURIComponent(licit.organo)}`,
...
function verDocumentos(licit) {
  ...
  switch (urlInfo.tipo) {
    ...
    case 'construida_expediente':
      ...
      consejo = '\n\n⚠️ El enlace puede no funcionar si el portal ha cambiado.';
      break;
    case 'busqueda_organo':
      mensaje = `🔎 Abriendo búsqueda por órgano: ${licit.organo}`;
      consejo = '\n\n⚠️ Busca la licitación por título o expediente.';
      break;
    default:
    ...
    <div id="modalDocumentos" class="modal-overlay" style="display:none;z-index:1000" onclick="cerrarModalDocumentos(event)">
      <div class="modal" style="max-width:700px;width:90%" onclick="event.stopPropagation()">
        <div class="modal-head">
          ...
          <button class="modal-close" onclick="cerrarModalDocumentos()"></button>
        </div>
...
async function verDocumentosScraping(licit) {
  ...
  if (!urlInfo || !urlInfo.url) {
    document.getElementById('modalDocBody').innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-title">No hay enlace disponible</div>
        <div class="empty-sub">Esta licitación no tiene enlace asociado.</div>
      </div>
    `;
  ...
  document.getElementById('modalDocTitle').innerHTML = `📄 Documentos - ${(licit.objeto || 'Licitación').substring(0, 60)}...`;
  document.getElementById('modalDocBody').innerHTML = `
    <div style="text-align:center;padding:40px">
      <div class="ia-spinner"></div>
  ...
  try {
    ...
    if (!data.success || data.documentos.length === 0) {
      document.getElementById('modalDocBody').innerHTML = `
        <div class="empty-state">
          ...
          <div class="empty-sub">Portal: ${data.portal || 'Desconocido'}<br><br>
          <a href="${urlInfo.url}" target="_blank" rel="noopener" style="color:var(--accent)"> Abrir portal manualmente</a></div>
        </div>
      `;
      return;
    }
    ...
    const documentosHTML = `
      <div style="margin-bottom:12px">
        ...
        <div style="background:rgba(59,130,246,.08);padding:8px 12px;border-radius:6px;font-size:.75rem;color:var(--text2)">
          📍 Portal: ${data.portal}
        </div>
      </div>
      <div id="listaDocumentos">
        ${data.documentos.map((doc, i) => `
          <div class="documento-item" style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border)">
            <input type="checkbox" class="doc-checkbox" data-url="${doc.url}" data-nombre="${doc.nombre}" value="${doc.url}" style="margin-right:8px">
            <span style="font-size:1.2rem">${doc.tipo === 'PDF' ? '📄' : '🔗'}</span>
            <div style="flex:1">
              <div style="font-size:.85rem;font-weight:500;word-break:break-all">${doc.nombre}</div>
              <div style="font-size:.7rem;color:var(--text2);word-break:break-all">${doc.url.substring(0, 60)}${doc.url.length > 60 ? '...' : ''}</div>
            </div>
            <a href="${doc.url}" target="_blank" class="btn btn-ghost btn-sm" style="padding:4px 8px" title="Descargar individual"></a>
          </div>
        `).join('')}
      </div>
    `;
    ...
    if (btnZip && data.documentos.length > 0) {
    ...
    document.getElementById('modalDocBody').innerHTML = `
      <div class="empty-state">
        <div class="empty-icon"></div>
        <div class="empty-title">Error al cargar documentos</div>
        <div class="empty-sub">No se pudo conectar con el portal.</div>
      </div>
    `;
  }
}
...
const SEGUIMIENTOS_KEY = 'licitlab_seguimientos';
...
function mostrarToast(mensaje, tipo = 'info') {
  ...
  toast.innerHTML = `
    <div style="position:fixed;bottom:20px;right:20px;z-index:10000;
      ...
      <span>${tipo === 'success' ? '' : tipo === 'warning' ? '⚠️' : ''}</span>
      <span>${mensaje}</span>
    </div>
    <style>
      @keyframes slideIn {
      ...
      <div id="modalSeguimientos" class="modal-overlay" style="display:none;z-index:1001" onclick="cerrarModalSeguimientos(event)">
        <div class="modal" style="max-width:600px;width:90%" onclick="event.stopPropagation()">
          <div class="modal-head">
            ...
            <button class="modal-close" onclick="cerrarModalSeguimientos()"></button>
          </div>
        ...
        ${Object.entries(seguimientos).map(([id, seg]) => `
          <div style="border:1px solid var(--border);border-radius:8px;padding:12px">
            <div style="display:flex;justify-content:space-between;align-items:flex-start">
              ...
              <button class="btn btn-ghost btn-sm" onclick="eliminarSeguimiento('${id}'); location.reload()" style="color:var(--red)">🗑️</button>
            </div>
          </div>
        `).join('')}
      </div>
  ...
  const modalHTML = `
    <div id="modalGenerador" class="modal-overlay" style="display:none;z-index:1002" onclick="cerrarModalGenerador(event)">
      <div class="modal" style="max-width:800px;width:90%" onclick="event.stopPropagation()">
        <div class="modal-head">
          <div class="modal-title">✍️ Asistente IA - Generar documento</div>
          <button class="modal-close" onclick="cerrarModalGenerador()"></button>
        </div>
    ...
    <div style="margin-bottom:20px">
      ...
      <div style="background:var(--surface2);padding:12px;border-radius:8px;font-size:.85rem">
        ...
        <div style="margin-top:4px;color:var(--text2);font-size:.75rem">
          ${licit.organo ? `🏛️ ${escHtml(licit.organo)}` : ''}
...
async function seleccionarTipoDocumento(tipoId, tipoNombre) {
  ...
  try {
    const contenido = await generarDocumentoConIA(licitacionActual, tipoId, tipoNombre);
    contenidoGenerado = contenido;
    preview.innerHTML = formatoTextoParaHTML(contenido);
    spinner.style.display = 'none';
    document.getElementById('btnRegenerar').style.display = 'inline-block';
    document.getElementById('btnDescargarDoc').style.display = 'inline-block';
  } catch (error) {
    console.error('Error generando documento:', error);
    preview.innerHTML = `<div style="color:var(--red)">❌ Error al generar el documento. Intenta de nuevo.</div>`;
    spinner.style.display = 'none';
  }
}

async function generarDocumentoConIA(licit, tipoDoc, tipoNombre) {
  const importe = licit._importeConIVA || parseAmount(licit.presupuestoConIVA) || 0;
  
  const prompts = {
    presentacion: `Genera una **Carta de presentación profesional** para esta licitación pública. Formato: Fecha, a quién va dirigida, presentación de la empresa, interés en la licitación, compromiso de calidad, firma. Tono formal. Máximo 300 palabras.`,
    tecnica: `Genera una **Oferta Técnica** estructurada: 1. Resumen ejecutivo, 2. Metodología de trabajo, 3. Equipo propuesto, 4. Garantías de calidad. 400-600 palabras.`,
    economica: `Genera una **Oferta Económica** detallada: cuadro de precios, gastos asociados, condiciones de pago, total. Presupuesto base: ${formatEUR(importe)}.`,
    anexos: `Genera lista de **Anexos requeridos**: documentación administrativa, certificados, declaraciones responsables, documentación técnica. Formato lista.`,
    cv: `Genera perfiles profesionales del equipo (3-4 roles): rol, experiencia, tecnologías clave, proyectos relevantes.`
  };
  
  const prompt = `Eres un asistente especializado en redacción de documentación para licitaciones. DATOS: Objeto: ${groqSanear(licit.objeto)}, Órgano: ${groqSanear(licit.organo)}, CPV: ${groqSanear(licit.cpv)}, Presupuesto: ${importe ? formatEUR(importe) : 'No especificado'}. TIPO: ${tipoNombre}. ${prompts[tipoDoc] || prompts.presentacion} IMPORTANTE: Usa español profesional. La empresa se llama [Nombre de tu empresa]. Devuelve SOLO el contenido del documento.`;
  
  try {
    const response = await fetch('/api/ia/groq', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        messages: [{ role: 'user', content: prompt }],
    ...
    const data = envelope.data ?? envelope;
    return data.choices?.[0]?.message?.content?.trim() || 'No se pudo generar el contenido.';
  } catch (error) {
    console.error('[Documentos IA] Error:', error);
    throw error;
  }
}

function formatoTextoParaHTML(texto) {
  return texto
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
function regenerarDocumento() {
  if (licitacionActual && window.tipoDocSeleccionado) {
...
function descargarDocumento() {
  ...
  const url = URL.createObjectURL(blob);
  link.href = url;
  link.download = `documento_${new Date().toISOString().slice(0,19)}.txt`;
  link.click();
  URL.revokeObjectURL(url);
