...
// ============================================================
//  SISTEMA GLOBAL DE NOTIFICACIONES DE ERROR DE RED
//  showToast(msg, tipo)  — muestra un toast en la esquina
//  apiFetch(url, opts)   — wrapper de fetch con manejo automático
// ============================================================
 ...
 * @param {string} msg    - Detalle del mensaje
 * @param {'error'|'warning'|'success'|'info'} tipo
 * @param {number} ms     - Duración en ms (0 = no cierra solo)
 */
function showToast(title, msg = '', tipo = 'error', ms = 5000) {
  const toast  = document.getElementById('networkToast');
  const tTitle = document.getElementById('networkToastTitle');
  const tMsg   = document.getElementById('networkToastMsg');
  const tIcon  = document.getElementById('networkToastIcon');
  if (!toast) return;

  const config = {
    error:   { icon: '⚠️', border: '#e74c3c' },
    warning: { icon: '🔔', border: '#f39c12' },
    success: { icon: '✅', border: '#27ae60' },
    info:    { icon: 'ℹ️',  border: '#3498db' },
  };
  const { icon, border } = config[tipo] || config.error;
  ...
  if (ms > 0) {
    _toastTimer = setTimeout(() => { toast.style.display = 'none'; }, ms);
  }
}

/**
 * Wrapper de fetch con manejo global de errores de red y sesión.
 * Úsalo igual que fetch(): apiFetch('/api/algo', { method:'POST', body:... })
 *
 * Comportamiento automático:
 *  - Sin red / servidor caído → toast de error + lanza excepción
 *  - HTTP 401 → toast "sesión expirada" + redirige a login (2 s)
 *  - HTTP 403 → toast "sin permisos"
 *  - HTTP 5xx → toast con el mensaje del servidor
 */
async function apiFetch(url, opts = {}) {
  ...
  try {
    ...
    // El servidor no respondió (caído, sin red, CORS bloqueado)
    showToast(
  ...
  // 401 — sesión expirada o no iniciada
  ...
  if (response.status >= 500) {
    ...
    showToast('Error del servidor', detail, 'error', 6000);
    // No lanzamos excepción aquí para que el llamador pueda
    // seguir procesando la respuesta si lo necesita.
  }

  return response;
}

// ── Detector de pérdida de red del navegador ─────────────────
window.addEventListener('offline', () => {
  showToast(
    'Sin conexión a Internet',
    'Parece que has perdido la conexión. Algunas funciones no estarán disponibles.',
    'warning',
    0
  );
});
window.addEventListener('online', () => {
  showToast('Conexión restaurada', 'Ya tienes conexión a Internet.', 'success', 3000);
});